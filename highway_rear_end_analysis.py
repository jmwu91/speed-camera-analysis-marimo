...
app = marimo.App(
    width="full",
    app_title="Speed Camera Effectiveness — Interactive Analysis with marimo",
)
...
def _(mo):
    ...
    This interactive notebook evaluates the effectiveness of speed cameras on
    provincial highways by comparing accident rates **before and after** camera
    installation. It demonstrates how **marimo** enables stakeholders (PMs, 
    policymakers) to explore analysis results interactively — adjusting 
    parameters, selecting sites, and inspecting spatial distributions on a map.

    ## Key Features of this marimo Notebook
    - **Reactive parameters** — change buffer distance / analysis period and 
      results update automatically
    - **Interactive table** — click rows to drill down into individual sites
    - **Live map** — spatial overlay of cameras, accidents, and buffer zones
    - **Dashboard cards** — summary statistics rendered as styled HTML

    ## Methodology
    - **Spatial Analysis**: Buffer zones around each camera → spatial join 
      with accident records (GeoPandas)
    - **Rate-Based Comparison**: Monthly accident rate before vs. after
    - **Statistical Test**: Mann–Whitney U test for significance (p < 0.05)

    > **Note**: Camera locations are **real public data** from Pingtung County.
    > Accident records are **synthetic** for demonstration purposes — the data
    > structure mirrors real government datasets but all accident values are fictional.
    """)
    return
...
def _(mo):
    buffer_dist_input = mo.ui.number(
        ...
        stop=500,
        step=10,
        label="Buffer Distance (m) / 緩衝區距離 (公尺)",
    )
    ...
    target_months_input = mo.ui.number(
        ...
        stop=60,
        step=6,
        label="Analysis Period (months) / 分析期間 (月)",
    )
    ...
    min_post_months_input = mo.ui.number(
        ...
        stop=12.0,
        step=0.5,
        label="Min Post-Install Period (months) / 最低設置後期間 (月)",
    )
...
def _(
    ...
    mo.md(f"""
    ### Data Processing Complete / 資料處理完成
    - **Unique accidents / 唯一事故數**: {len(df_acc_unique):,}
    - **Geocoded accidents / 有座標事故**: {len(gdf_acc):,}
    - **Data period / 資料期間**: {MIN_ACC_DATE.date()} ~ {MAX_ACC_DATE.date()}
    - **Buffer distance / 緩衝區距離**: {BUFFER_DIST} m
    """)
    return (
        MAX_ACC_DATE,
        MIN_ACC_DATE,
...
def _(BUFFER_DIST, gdf_acc, gdf_cam, gpd, mo):
    ...
    mo.md(f"""
    ### Spatial Join Results / 空間連結結果
    - **Buffer distance / 緩衝區距離**: {BUFFER_DIST} m
    - **Matched accidents / 匹配事故數**: {len(joined):,}
    """)
    return (joined,)
...
def _(df_results, mo, pd):
    ...
    status_df = pd.DataFrame(
        {"Status": status_counts.index, "Count / 數量": status_counts.values}
    )

    mo.vstack([mo.md("### Device Status / 設備狀態分布"), mo.ui.table(status_df, selection=None)])
    return
...
def _(df_results, mo):
    ...
    <div style="font-family: 'Segoe UI', sans-serif; background: #f8f9fa; padding: 20px; border-radius: 10px;">
        <h3 style="margin-top: 0; color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px; display: inline-block;">Accident Analysis Dashboard</h3>

        <div style="margin-top: 15px; margin-bottom: 20px; color: #666;">
            <strong>Sample:</strong> {n_valid} valid devices / {n_with_acc} with accident records
        </div>

        <h4 style="color: #34495e; margin-bottom: 10px;">Accident Counts / 事故件數</h4>
        <div style="display: flex; gap: 15px; flex-wrap: wrap; margin-bottom: 25px;">
            {card("Avg Reduction Rate", acc_red, "%", "raw comparison", color_val(acc_red))}
            {card("Improved Sites", acc_imp_pct, "%", f"{acc_imp_cnt} / {n_with_acc}", color_val(acc_imp_pct-50))}
        </div>

        <h4 style="color: #34495e; margin-bottom: 10px;">Casualties / 死傷人數</h4>
        <div style="display: flex; gap: 15px; flex-wrap: wrap;">
            {card("Avg Reduction Rate", cas_red, "%", "raw comparison", color_val(cas_red))}
            {card("Improved Sites", cas_imp_pct, "%", f"{cas_imp_cnt} / {n_with_acc}", color_val(cas_imp_pct-50))}
        </div>
    </div>
    """
...
def _(mo):
    mo.md(f"""
    ---
    ### Sites with Accident Records / 有事故資料的設備
    """)
    return
...
def _(df_valid_with_acc, plt, sns):
    ...
    if len(df_valid_with_acc) > 0:
        ...
        axes1[0, 0].set_title("Accident Reduction Rate Distribution")
        ...
        axes1[0, 1].set_xlabel("Casualty Reduction Rate (%)")
        axes1[0, 1].set_title("Casualty Reduction Rate Distribution")
        ...
        axes1[1, 0].set_title(f"Accident: {acc_improved}/{len(df_valid_with_acc)} improved")
        ...
        axes1[1, 1].set_title(f"Casualty: {cas_improved}/{len(df_valid_with_acc)} improved")
...
def _(mo):
    mo.md(r"""
    ## Detailed Results Table / 詳細結果表格

    Select rows to view details and map below.
    """)
    return
...
def _(df_results, mo):
    ...
    mo.vstack([mo.md("### Site Data / 站點資料"), table_ui])
    return (table_ui,)
        ...
        for _, row in selected_df.iterrows():
            ...
            ">
                ...
                <div style="flex: 1; min-width: 140px; text-align: center; border-right: 1px solid #eee;">
                    <div style="font-size: 0.8em; color: #7f8c8d;">Accidents</div>
                    <div style="font-weight: bold; font-size: 1.1em; color: {acc_color};">
                        {int(row['Pre_Total'])} -> {int(row['Post_Total'])}
                    </div>
                    <div style="font-size: 0.75em; color: #95a5a6;">({row['Reduction_Rate_%']}%)</div>
                </div>

                <div style="flex: 1; min-width: 140px; text-align: center;">
                    <div style="font-size: 0.8em; color: #7f8c8d;">Casualties</div>
                    <div style="font-weight: bold; font-size: 1.1em; color: {cas_color};">
                        {int(row['Pre_Casualty'])} -> {int(row['Post_Casualty'])}
                    </div>
                    <div style="font-size: 0.75em; color: #95a5a6;">({row['Casualty_Reduction_%']}%)</div>
                </div>
            </div>
            """

        display_content = mo.vstack([
            mo.md(f"### Selected Site Details ({len(selected_df)})"),
            mo.Html(
                f"<div style='background: #f4f6f7; padding: 15px; border-radius: 8px;'>{cards_html}</div>"
            ),
        ])
    else:
        display_content = mo.md("_Select rows in the table above to see details_")
    ...
    if len(selected_df) > 0:
        ...
        for _, map_cam_row in selected_df.iterrows():
            ...
            folium.Marker(
                ...
                popup=f"<b>{map_cam_id}</b><br>{map_cam_row['Location']}<br>Setup: {str(map_setup_date)[:10]}",
            ...
            if (
                ...
                for _, map_acc in map_cam_accs.iterrows():
                    try:
                        ...
                        if map_is_pre:
                            map_color = "orange"
                            map_period = "Pre-install"
                        else:
                            map_color = "green"
                            map_period = "Post-install"

                        map_casualty = map_acc.get("Casualty", 0)
                        map_popup_text = f"<b>{map_period}</b><br>Date: {str(map_acc_date)[:10]}<br>Casualties: {int(map_casualty) if pd.notna(map_casualty) else 0}"
        ...
        <div style="position: fixed; bottom: 50px; left: 50px; z-index: 1000;
            ...
            <b>Legend</b><br>
            Blue marker = Speed camera<br>
            Orange dot = Pre-install accident<br>
            Green dot = Post-install accident
        </div>
        """
        ...
        map_display = mo.md("_Select rows in the table to show the map_")
...
def _(mo):
    ...
    This analysis examines provincial highway accidents potentially caused by
    speeding behavior, filtered by:

    1. **Accident types** likely caused by excessive speed:
       - 12: Same-direction sideswipe, 13: Rear-end collision, 18: Rollover,
         19-29: Various fixed-object collisions

    2. **Primary causes** indicating speed violations:
       - Pre-2023/07: codes 13 (speeding), 14 (failure to decelerate),
         16 (unsafe following distance), 17 (unsafe lateral distance)
       - Post-2023/07: codes 5, 6, 7, 8 (updated classification system)

    ### Metrics
    - **Reduction_Rate_%**: Raw accident rate reduction (monthly average, before vs. after)
    - **DiD_Net_Reduction_%**: Difference-in-Differences net effect (controlling for province-wide trends)
    - **Casualty**: Deaths + Injuries per accident
    - **Mann-Whitney U**: Non-parametric test for significance (p < 0.05)

    > **Demo Note**: This notebook uses synthetic data for demonstration.
    > The analysis framework and interactive features are production-ready
    > and have been used for real policy evaluation.
    """)
    return
